...
def analyze_command(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            "--config",
            help="Path to TOML configuration file (defaults to app data directory)",
        ),
    ] = None,
    sharpness_threshold: Annotated[
        float | None,
        typer.Option(
            "--sharpness-threshold",
            help="Override sharpness threshold for quality assessment",
        ),
    ] = None,
    brightness_min: Annotated[
        float | None,
        typer.Option(
            "--brightness-min",
            help="Override minimum brightness threshold",
        ),
    ] = None,
    brightness_max: Annotated[
        float | None,
        typer.Option(
            "--brightness-max",
            help="Override maximum brightness threshold",
        ),
    ] = None,
    contrast_threshold: Annotated[
        float | None,
        typer.Option(
            "--contrast-threshold",
            help="Override contrast threshold for quality assessment",
        ),
    ] = None,
    face_confidence_threshold: Annotated[
        ...
        typer.Option(
            ...
            help="Override face detection confidence threshold (0.0 to 1.0)",
        ),
    ] = None,
    quality_weight: Annotated[
        ...
        typer.Option(
            "--quality-weight",
            help="Override quality component weight in scoring (0.0 to 1.0)",
        ),
    ] = None,
    face_weight: Annotated[
        float | None,
        typer.Option(
            "--face-weight",
            help="Override face component weight in scoring (0.0 to 1.0)",
        ),
    ] = None,
) -> None:
    ...
    if sharpness_threshold is not None:
        config.quality.sharpness_threshold = sharpness_threshold
    if brightness_min is not None:
        config.quality.brightness_min = brightness_min
    if brightness_max is not None:
        config.quality.brightness_max = brightness_max
    if contrast_threshold is not None:
        config.quality.contrast_threshold = contrast_threshold
    if face_confidence_threshold is not None:
        config.face.confidence_threshold = face_confidence_threshold
    if quality_weight is not None:
        config.scoring.quality_weight = quality_weight
    if face_weight is not None:
        config.scoring.face_weight = face_weight

    # Validate weight configuration
    if quality_weight is not None or face_weight is not None:
        ...
        if abs(total_weight - 1.0) > 0.01:  # Allow small floating point differences
            console.error(
                f"Quality and face weights must sum to 1.0, got {total_weight}"
            )
            raise typer.Exit(1)

    # Set up configuration manager
    config_manager = ConfigManager.get_instance()
    config_manager._config = config  # type: ignore[attr-defined]
...
def _display_analysis_summary(
    analysis: DirectoryAnalysis, config: CuLoRAConfig
) -> None:
    """Display detailed analysis results for all images.

    Args:
        analysis: DirectoryAnalysis results to display.
        config: Analysis configuration for display formatting.
    """
    ...
    for image in sorted_images:
        ...
        for stage in enabled_stages:
            if stage == AnalysisStage.QUALITY:
                ...
                row_data.extend(_format_quality_data(quality_result, config.display))
            elif stage == AnalysisStage.FACE:
        ...
        # Add score (always shown) with configurable color thresholds
        score_str = f"{image.score:.2f}"
        display_config = config.display
        if image.score >= display_config.score_excellent_threshold:
            score_str = f"[green]{score_str}[/green]"
        elif image.score >= display_config.score_good_threshold:
...
def _format_quality_data(
    quality_result: ImageQualityResult | None, display_config: DisplayConfig
) -> list[str]:
    """Format quality result data into separate column values.

    Args:
        quality_result: Quality analysis result or None
        display_config: Display configuration for color thresholds
    ...
    if quality_result.sharpness_score >= display_config.sharpness_display_excellent:
        sharpness_str = f"[green]{sharpness_str}[/green]"
    elif quality_result.sharpness_score >= display_config.sharpness_display_good:
        sharpness_str = f"[yellow]{sharpness_str}[/yellow]"
    else:
        sharpness_str = f"[red]{sharpness_str}[/red]"

    if (
        display_config.brightness_display_min
        <= quality_result.brightness_score
        <= display_config.brightness_display_max
    ):
        brightness_str = f"[green]{brightness_str}[/green]"
    else:
        brightness_str = f"[yellow]{brightness_str}[/yellow]"

    if quality_result.contrast_score >= display_config.contrast_display_excellent:
        contrast_str = f"[green]{contrast_str}[/green]"
    elif quality_result.contrast_score >= display_config.contrast_display_good:
        contrast_str = f"[yellow]{contrast_str}[/yellow]"
    else:
        contrast_str = f"[red]{contrast_str}[/red]"

    # Use same thresholds as score for composite quality display
