...
class ImageAnalyzer:
    ...
    def analyze_directory(self, input_directory: Path) -> DirectoryAnalysis:
        ...
        if not image_paths:
            return DirectoryAnalysis(
                ...
                analysis_config=self._config_manager.analysis_config,
                images=[],
            )
    ...
    def _calculate_image_score(
        ...
        # Quality component (50% weight) - increased importance
        ...
        if quality_result:
            ...
            score += quality_component * 0.5

        # Face component (50% weight)
        ...
        if face_result and face_result.face_count > 0:
            ...
            # Face area scoring with sigmoid-like curve for diminishing returns
            # Ideal face area: 5-25% of image, with peak around 15%
            if face_area_ratio < 0.05:
                # Too small
                area_score = face_area_ratio / 0.05  # Linear up to 5%
            elif face_area_ratio <= 0.15:
                # Sweet spot - linear growth to peak
                area_score = 0.5 + (face_area_ratio - 0.05) * 5.0  # Peaks at 1.0 at 15%
            elif face_area_ratio <= 0.25:
                # Good but declining
                area_score = (
                    1.0 - (face_area_ratio - 0.15) * 2.0
                )  # Declines to 0.8 at 25%
            else:
                # Too large
                area_score = max(0.2, 0.8 - (face_area_ratio - 0.25) * 2.0)
            ...
            # Face count penalty: slight penalty for multiple faces
            if face_result.face_count > 1:
                # 10% penalty for each additional face, max 50% penalty
                penalty_factor = max(0.5, 1.0 - (face_result.face_count - 1) * 0.1)
                face_component *= penalty_factor

            score += face_component * 0.5
